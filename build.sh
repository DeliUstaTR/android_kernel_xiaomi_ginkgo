...
make -j$(nproc --all) O=out ARCH=arm64 CC=clang LD=ld.lld AR=llvm-ar AS=llvm-as NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip CROSS_COMPILE=clang CROSS_COMPILE_ARM32=clang CLANG_TRIPLE=aarch64-linux-gnu- Image.gz-dtb dtbo.img
